...
import com.google.location.lbs.gnss.gps.pseudorange.Constants.GloConstants;
...
public class GloEphemeris extends GnssEphemeris {
  ...
  public int newerThan(GnssEphemeris other) {
    ...
    Preconditions.checkArgument(
        (other instanceof GloEphemeris), "Expecting the input to be an instance of GloEphemeris");
    GloEphemeris otherGloEph = (GloEphemeris) other;
    return utcTime.compareTo(otherGloEph.utcTime);
  }

  @Override
  public boolean isExpired(GnssTime gnssTimeOfInterest) {
    return Math.abs(computeElapsedSecondsFromToe(gnssTimeOfInterest))
        > GloConstants.GLO_EPH_VALID_PERIOD_SECONDS / 2;
  }

  @Override
  public double computeElapsedSecondsFromToe(GnssTime gnssTimeOfInterest) {
    DateTimePicos utcTimeOfInterest = gnssTimeOfInterest.toUtcTime();
    return utcTimeOfInterest.computeElapsedMillisFrom(utcTime) * TimeConstants.SECONDS_PER_MILLI;
  }
