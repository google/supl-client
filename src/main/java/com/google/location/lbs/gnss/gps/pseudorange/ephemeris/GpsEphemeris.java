...
public class GpsEphemeris extends KeplerianEphemeris {
  ...
  public int newerThan(GnssEphemeris other) {
    ...
    Preconditions.checkArgument(
        (other instanceof GpsEphemeris), "Expecting the input to be an instance of GpsEphemeris");
    GpsEphemeris otherGpsEph = (GpsEphemeris) other;
    return ComparisonChain.start()
        .compare(week, otherGpsEph.week)
        .compare(keplerModel.toeS, otherGpsEph.keplerModel.toeS)
        .result();
  }
  ...
  public boolean isExpired(GnssTime gnssTimeOfInterest) {
    return Math.abs(computeElapsedSecondsFromToe(gnssTimeOfInterest))
        > GpsConstants.GPS_EPH_VALID_PERIOD_SECONDS / 2;
  }
